...
class DataValidation:
    ...
    def validate_schema(self, dataframe: pd.DataFrame) -> bool:
        try:
            expected_columns = list(self.schema_config.keys())
            ...
            # 2. Since count matches, now check if names are the same
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            status=self.validate_schema(dataframe=train_dataframe)
            if not status:
                error_message=f"Train dataframe does not contain all columns.\n"
            status = self.validate_schema(dataframe=test_dataframe)
            if not status:
